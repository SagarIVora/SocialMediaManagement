...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        private readonly IRepository<UserMobileOTP> _mobileOtp;
        private readonly IRepository<UserEmailOtp> _mailOtp;
        ...
        public CustomerController(AddressSettings addressSettings,
            ...
            IRepository<UserEmailOtp> mailOtp,
            IRepository<UserMobileOTP> mobileOtp,
        ...
        public async Task<IActionResult> SendLoginViaOtpVerification(string email)
        {
            try
            {
                #region Mail
                ...
                client.Credentials = new NetworkCredential("hamzacodedot@gmail.com", "ictxhazegcfsomib");

                var OTP = GenerateRandomOTP(6);
                var message = $"Your OTP is: {OTP}. Please Enetr this code to Login To Your Account.";
                using (var mailmessage = new MailMessage())
                {
                    mailmessage.From = new MailAddress("hamzacodedot@gmail.com");
                    mailmessage.To.Add(email);
                    mailmessage.Subject = "OTP for Login to Your Account";
                ...
                var existingotp = _mailOtp.GetAll().Where(x => x.Email == email).ToListAsync().Result.FirstOrDefault();
                if (existingotp == null)
                {
                    UserEmailOtp emailOtpDATA = new UserEmailOtp();
                    emailOtpDATA.Email = email;
                    emailOtpDATA.OTP = OTP;
                    await _mailOtp.InsertAsync(emailOtpDATA);
                }
                else
                {
                    existingotp.OTP = OTP;
                    await _mailOtp.UpdateAsync(existingotp);
                }
                #endregion

                var mobileNumber = _customerService.GetCustomerByEmailAsync(email).Result.Phone;

                #region SendMessage
                if (mobileNumber != null && mobileNumber != "")
                {
                    string accountSid = "AC9d9aa5f4cf0c4cb14cb676ef4af7ed27";
                    string authToken = "3e311cfb040f7f85b10d5811bd857ab8";
                    string twilioPhoneNumber = "+14155238886";
                    ...
                    var messageOptions = new CreateMessageOptions(
                        new PhoneNumber($"whatsapp:{mobileNumber}"))
                    {
                    ...
                    var existingotpmob = _mobileOtp.GetAll().Where(x => x.PhoneNumber.Contains(mobileNumber) || x.PhoneNumber == mobileNumber).ToListAsync().Result.FirstOrDefault();
                    if (existingotp == null)
                    {
                        UserMobileOTP mobileOtpData = new UserMobileOTP();
                        mobileOtpData.PhoneNumber = mobileNumber;
                        mobileOtpData.OTP = OTP;
                        await _mobileOtp.InsertAsync(mobileOtpData);
                    }
                    else
                    {
                        existingotpmob.OTP = OTP;
                        await _mobileOtp.UpdateAsync(existingotpmob);
                    }
                }
                #endregion
                return Json(true);
        ...
        public virtual async Task<IActionResult> LoginViaOTP(string email, string OTP)
        {
            var customerEmail = email?.Trim();

            var existingotp = _mailOtp.GetAll().Where(x => x.Email == email).ToListAsync().Result.FirstOrDefault().OTP;

            if (existingotp == OTP)
            {
                var customer = await _customerService.GetCustomerByEmailAsync(customerEmail);
                return await _customerRegistrationService.SignInCustomerAsync(customer, "/", false);
            }
            else
            {
        ...
        public async Task<IActionResult> SendVerificationEmail(string email)
        {
            try
            {
                ...
                {
                    message.From = new MailAddress("hamzacodedot@gmail.com");
                    message.To.Add(email);
                    message.Subject = "User Email Verification";
                    message.Body = $"Your OTP is: {OTP}. Please Enetr this code for verification of your Email.";
        ...
        public async Task<IActionResult> SendWhatsAppMessage(string to)
        {
            try
            {
                var accountSid = "AC9d9aa5f4cf0c4cb14cb676ef4af7ed27";
                var authToken = "3e311cfb040f7f85b10d5811bd857ab8";
                var twilioPhoneNumber = "+14155238886";
                ...
                var message = $"Your OTP is: {OTP}. Please Enetr this code for verification of your Phone Number.";
                ...
                var existingotp = _mobileOtp.GetAll().Where(x => x.PhoneNumber.Contains(to) || x.PhoneNumber == to).ToListAsync().Result.FirstOrDefault();
                if (existingotp == null)
                {
                    UserMobileOTP mobileOtpData = new UserMobileOTP();
                    mobileOtpData.PhoneNumber = to;
                    mobileOtpData.OTP = OTP;
                    await _mobileOtp.InsertAsync(mobileOtpData);
                }
                else
                {
                    existingotp.OTP = OTP;
                    await _mobileOtp.UpdateAsync(existingotp);
                }
                return Json(true);
            }
            catch (Exception ex)
        ...
        [HttpGet]
        public async Task<IActionResult> VerifyMobileOtp(string number, string otp)
        {
            var existingotp = _mobileOtp.GetAll().Where(x => x.PhoneNumber.Contains(number) || x.PhoneNumber == number).ToListAsync().Result.FirstOrDefault().OTP;
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (model.VendorRole)
                    {
                        ...
                        vendor.Description = "New Vendor Creation";
                        vendor.Email = customerEmail;
                        vendor.AllowCustomersToSelectPageSize = true;
                        vendor.ManuallyPriceRange = true;
                        vendor.PriceRangeFiltering = true;
                        vendor.PageSize = 6;
                        vendor.PriceFrom = 0;
                        vendor.PriceTo = 10000;
                        vendor.PageSizeOptions = "6,3,9";

                        var vendor2 = vendor.ToEntity<Vendor>();
                        await _vendorService.InsertVendorAsync(vendor2);
                        customer.VendorId = vendor2.Id;
                    }
                    //VAT number
        ...
        public async Task<IActionResult> InstaPost(string ImageUrl)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        var image = CaptureWebPageScreenshot(ImageUrl);
                        return Ok(new { imageBase64 = $"data:image/png;base64,{image}" });
                    }
                    else
                    {
                        return BadRequest(new { error = "Failed to load the image" });
                    }
                }
            }
            catch (Exception)
        ...
        private string CaptureWebPageScreenshot(string url)
        {
            try
            {
                using (IWebDriver driver = new ChromeDriver())
                {
                    ...
                    var screenshotImage = screenshot.AsBase64EncodedString;
        ...
        public async Task<IActionResult> InstaProfile(string username)
        {
            var postUrl = "https://www.instagram.com/" + username + "/";
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        var image = CaptureWebPageScreenshot(postUrl);
                        return Ok(new { imageBase64 = $"data:image/png;base64,{image}" });
                    }
                    else
                    {
